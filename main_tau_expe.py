...
def main():
    ...
    parser.add_argument('--model', type=str, required=True, help='Model architecture from model_zoo (e.g., CNNA_CIFAR10_1_LIP_Bjork)')
    ...
    parser.add_argument('--output_plot', type=str, default='results/era_vs_cra_trajectory.png', help='Path for ERA(i+1) vs CRA(i) plot')
    # NEW ARGUMENT: Output path for the new same-tau plot
    parser.add_argument('--output_plot_same_tau', type=str, default='results/era_vs_cra_same_tau.png', help='Path for ERA(i) vs CRA(i) plot')
    parser.add_argument('--tau_step', type=int, default=2, help='Step size to skip temperatures (e.g., 2 to take half the points)')
    ...
    os.makedirs(os.path.dirname(args.output_plot_same_tau), exist_ok=True)

    # --- 2. LOAD DATASET ---
    images, targets, classes = load_dataset_benchmark_auto(args)
    print(f"Dataset loaded: {len(targets)} samples.")

    # --- 3. PARSE MODELS IN DIRECTORY ---
    search_pattern = os.path.join(args.models_dir, "*.pth")
    all_files = glob.glob(search_pattern)
    
    print(f"DEBUG: Searching directory: {args.models_dir}")
    print(f"DEBUG: Found {len(all_files)} total '.pth' files.")
    ...
    for mf in all_files:
        ...
        # Only process files that start with our target model name
        ...
        if match:
            ...
            print(f"Warning: Found model {filename} but couldn't parse temperature (T).")
            
    # Sort models by temperature (tau) in ascending order
    tau_models.sort(key=lambda x: x[0])

    # Subsample the models to increase the gap
    if args.tau_step > 1:
        original_count = len(tau_models)
        tau_models = tau_models[::args.tau_step]
        print(f"Subsampled models using step={args.tau_step}. Reduced points from {original_count} to {len(tau_models)}.")
    
    if not tau_models:
        print(f"No models found starting with '{target_prefix}' that have a valid '_T<val>_' tag. Exiting.")
        return

    print(f"Found {len(tau_models)} models for architecture '{args.model}'. Evaluating at fixed epsilon: {args.eps:.5f}")

    results = []

    # --- 4. EVALUATION LOOP ---
    for i, (tau, model_path) in enumerate(tau_models):
        print(f"\n[{i+1}/{len(tau_models)}] Evaluating model with Temperature T={tau}: {os.path.basename(model_path)}")
        ...
        try:
            ...
            print(f"  > Warning: Power Iteration failed ({e}). Using Theoretical L.")
            L = L_theory

        # Scaling logic for CIFAR/Imagenette
        ...
        # --- A. Empirical Robust Accuracy (AutoAttack) ---
        ...
        print(f"  > CRA (Certificates): {cra_val:.2f}%")
    ...
    print(f"Raw data saved to {args.output_csv}")
    ...
    cleans_sorted = sorted_df['clean_acc'].values
    ...
    # Safely split the output path to append suffixes
    base_dir = os.path.dirname(args.output_plot)
    base_name, ext = os.path.splitext(os.path.basename(args.output_plot))
    if not ext:
        ext = '.png'

    # ==========================================
    # PLOT 1: ERA, CRA, and Clean Acc vs ALL Temperatures
    # ==========================================
    plt.figure(figsize=(10, 7))

    plt.plot(taus_sorted, cleans_sorted, marker='^', markersize=8, linestyle='-', color='dodgerblue', linewidth=2, label='Clean Accuracy')
    plt.plot(taus_sorted, eras_sorted, marker='o', markersize=8, linestyle='-', color='crimson', linewidth=2, label='Empirical Robust Acc (ERA)')
    plt.plot(taus_sorted, cras_sorted, marker='s', markersize=8, linestyle='-', color='teal', linewidth=2, label='Certified Robust Acc (CRA)')
    ...
    plt.ylabel('Accuracy (%)')
    plt.title(f'Robust Accuracy vs Temperature\nModel: {args.model} | Dataset: {args.dataset} | Epsilon: {args.eps:.4f}')
    plt.legend(loc='best')
    plt.grid(True, linestyle=':', alpha=0.7)

    output_plot_main = os.path.join(base_dir, f"{base_name}_vs_tau_all{ext}")
    ...
    if has_potential:
        ...
        plt.plot(zoom_df['tau'], zoom_df['clean_acc'], marker='^', markersize=9, linestyle='-', color='dodgerblue', linewidth=2, label='Clean Acc')
        ...
        plt.xlabel('Temperature (Tau)')
        plt.ylabel('Accuracy (%)')
        plt.title('Zoomed-In: Robust Accuracy in Potential Region')
        plt.legend(loc='best')
        plt.grid(True, linestyle='--', alpha=0.7)
    ...
    # ==========================================
    # PLOT 3: Delta Plot (ERA - Max CRA) (LINEAR SCALE)
    # ==========================================
    ...
    plt.title('Performance Delta: Does ERA beat the best overall CRA?')
    ...
    if has_potential:
        ...
        for index, row in potential_df.iterrows():
            print(f"  > Tau: {row['tau']} | ERA: {row['era']:.2f}% | CRA: {row['cra']:.2f}% | Delta: +{(row['era'] - max_cra):.2f}%")
