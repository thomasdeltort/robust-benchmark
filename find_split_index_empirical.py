...
Includes strict OOM protection and resume functionality.
"""
...
def profile_true_vras(args, model_zoo):
    ...
    inp_shape = (1, 1, 28, 28) if "mnist" in args.dataset.lower() else (1, 3, 32, 32)
    ...
    if is_gnp:
        print("GNP Model detected. Bypassing empirical L estimation. L_full = 1.0")
        L_full = 1.0
    else:
    ...
    plot_save_path = f"true_vra_curves_candidates_{args.model}_tau_{args.high_tau}.png"

    # ======================================================================
    # 5. HELPER FUNCTION TO SAVE PROGRESS DYNAMICALLY
    # ======================================================================
    def save_current_progress(eval_indices, a_vras, s_vras):
        """Saves the JSON data and overwrites the plot with the current state."""
        # --- Save JSON ---
        results_dict = {
            "model": args.model,
        ...
        plt.plot(eval_indices, a_vras, marker='s', linestyle='--', color='orange', linewidth=2, label='Alpha-CROWN Hybrid VRA')
        if args.sdp and len(s_vras) == len(eval_indices):
            plt.plot(eval_indices, s_vras, marker='^', linestyle='-.', color='green', linewidth=2, label='SDP-CROWN Hybrid VRA')
        
        plt.axhline(y=cra_acc, color='blue', linestyle='-', linewidth=2, label=f'Global 1-Lip CRA ({cra_acc:.2f}%)')
        plt.xticks(eval_indices)
        ...
        plt.close() # <--- CRITICAL: Prevents OOM by closing the figure in the background
        
        print(f"\n[+] Progress saved: Updated '{plot_save_path}' and JSON file.")

    # ======================================================================
    # 6. ITERATE THROUGH CANDIDATE SPLITS AND COMPUTE VRA
    # ======================================================================
    print("\n" + "="*80)
    ...
    if os.path.exists(data_save_path):
        print(f"\n[*] Found existing progress file at '{data_save_path}'. Attempting to resume...")
        try:
            with open(data_save_path, 'r') as f:
                saved_data = json.load(f)
            evaluated_indices = saved_data.get("evaluated_indices", [])
            alpha_vras = saved_data.get("alpha_vras", [])
            sdp_vras = saved_data.get("sdp_vras", [])
            print(f"[*] Successfully loaded {len(evaluated_indices)} previously evaluated layers: {evaluated_indices}")
        except Exception as e:
            print(f"[!] Error reading JSON: {e}. Starting from scratch.")
            evaluated_indices, alpha_vras, sdp_vras = [], [], []

    print(f"Candidate indices to process: {sorted(candidate_indices, reverse=True)}")
    
    for k in sorted(candidate_indices, reverse=False)[1:]:
        # Skip if already evaluated
        if k in evaluated_indices:
            print(f"\n--- Skipping Candidate Split at Layer {k} (Already Computed) ---")
            continue
        ...
        if k == len(all_layers):
            ...
            save_current_progress(evaluated_indices, alpha_vras, sdp_vras)
            continue
        ...
        # Save progress dynamically at the end of this iteration
        save_current_progress(evaluated_indices, alpha_vras, sdp_vras)
