...
Alpha-CROWN, SDP-CROWN, and pure 1-Lipschitz CRA across all candidate split layers.
Candidate layers are restricted to those immediately preceding an activation.
Includes strict OOM protection. Automatically adapts to sequential models (VGG) 
and macro-block models (ResNets).
"""
...
def flatten_resnet(model):
    """
    Flattens a ResNetLipschitz model into a sequential list of modules.
    Treats entire BasicBlockLipschitz modules as single sequential steps
    so they can be safely processed by torchlip.Sequential.
    """
    flat_layers = []
    
    # 1. Initial Stem
    flat_layers.append(model.conv1)
    flat_layers.append(model.bc1)
    flat_layers.append(model.act)
    if not isinstance(model.pool1, nn.Identity):
        flat_layers.append(model.pool1)
        
    # 2. Residual Blocks
    for layer_group in model.layers:
        for block in layer_group:
            flat_layers.append(block)
            
    # 3. Classifier Head
    flat_layers.append(model.pool)
    flat_layers.append(nn.Flatten(1)) # Replaces functional torch.flatten
    flat_layers.append(model.fc)
...
def profile_true_vras(args, model_zoo):
    ...
    # Detect if we are using a ResNet or a sequential VGG/Conv model
    if isinstance(full_model, ResNetLipschitz):
        print("--> Detected ResNet architecture. Using macro-flattening...")
        all_layers = flatten_resnet(full_model)
        
        for k in range(1, len(all_layers)):
            layer_before = all_layers[k-1]
            layer_after = all_layers[k]
            
            # For ResNets: Split safely after BatchCentering or after an entire BasicBlock.
            if isinstance(layer_before, (LirpaBatchCentering2D, BasicBlockLipschitz)):
                candidate_indices.append(k)
                print(f"[*] Layer {k:2d} added: {layer_before.__class__.__name__} -> {layer_after.__class__.__name__}")
                
    else:
        print("--> Detected VGG/Sequential architecture. Using standard filtering...")
        all_layers = list(full_model.children())
        ...
        for k in range(1, len(all_layers)):
            ...
            # A valid candidate splits AFTER a Conv/Linear and BEFORE an Activation
            if isinstance(layer_before_split, (nn.Conv2d, nn.Linear)):
                if not isinstance(layer_after_split, (nn.Conv2d, nn.Linear, nn.Flatten)):
                    candidate_indices.append(k)
                    print(f"[*] Layer {k:2d} added: {layer_before_split.__class__.__name__} -> {layer_after_split.__class__.__name__}")
    ...
    print(f"Evaluating candidate indices (reverse order): {sorted(candidate_indices, reverse=True)}")
    ...
    # ======================================================================
    # 7. SAVE DATA AS A DICTIONARY (JSON)
    # ======================================================================
    results_dict = {
        "model": args.model,
