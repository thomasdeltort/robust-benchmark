...
#            
#            if torch.isnan(loss):
#                print(f"\n[ERROR] NaN loss at Epoch {epoch+1}, Step {step}. Experiment aborted.")
#                sys.exit(1)
#
#            loss.backward()
#
#            # --- DIAGNOSTIC LOGS: Targeting Parametrization for Real Gradients ---
#            if step % 100 == 0:
#                with torch.no_grad():
#                    l_std = output.std().item()
#                    g_first, g_last = 0.0, 0.0
#                    
#                    base = model.module if hasattr(model, 'module') else model
#
#                    # Parametrized layers store 'original' weights in a specific dictionary
#                    try:
#                        # Find grad for conv1
#                        if hasattr(base.conv1, 'parametrizations'):
#                            # torchlip/orthogonium usually store the unconstrained weight here
#                            g_first = base.conv1.parametrizations.weight[0].original.grad.norm().item()
#                        elif base.conv1.weight.grad is not None:
#                            g_first = base.conv1.weight.grad.norm().item()
#                        
#                        # Find grad for fc
#                        if hasattr(base.fc, 'parametrizations'):
#                            g_last = base.fc.parametrizations.weight[0].original.grad.norm().item()
#                        elif base.fc.weight.grad is not None:
#                            g_last = base.fc.weight.grad.norm().item()
#                    except Exception:
#                        pass # Keep as 0.0 if not found
#
#                    writer.add_scalar("Debug/Logit_Std", l_std, epoch * len(train_loader) + step)
#                    writer.add_scalar("Debug/Grad_Norm_FirstConv", g_first, epoch * len(train_loader) + step)
#                    writer.add_scalar("Debug/Grad_Norm_FC", g_last, epoch * len(train_loader) + step)
#
#                    if step == 0:
#                        # We use 'Real' in the log to distinguish from the virtual weight grad
#                        print(f"  [Batch 0] Logit Std: {l_std:.4f} | G_Real_First: {g_first:.2e} | G_Real_Last: {g_last:.2e}")
#
#            optimizer.step()
#
#            m_kr += kr_multiclass_loss(output, target_oh).item()
...
#        with torch.no_grad():
#            alphas = [torch.sigmoid(p).mean().item() for n, p in model.named_parameters() if 'alpha' in n]
#            if alphas:
#                avg_alpha = sum(alphas) / len(alphas)
#                writer.add_scalar("Debug/Avg_Alpha_Gate", avg_alpha, epoch)
#                print(f"  [Summary] Avg Alpha Gate: {avg_alpha:.4f}")
#        # -----------------------------
#
#        # --- Validation & CRA ---
#        val_metrics_str = ""
