...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        # --- Validation & CRA (Using Validation Loader) ---
        val_metrics_str = ""
        if (epoch + 1) % 10 == 0 or (epoch + 1) == args.epochs:
            model.eval() 
            optimizer.eval()

            val_output, val_targets, val_images = [], [], []
            with torch.no_grad():
                for data, target in val_loader:
                    val_images.append(data) 
                    out = model(data.to(device))
                    val_output.append(out.cpu())
                    val_targets.append(torch.nn.functional.one_hot(target, num_classes=num_classes))
            
            all_val_images = torch.cat(val_images)
            all_val_output = torch.cat(val_output)
            all_val_targets = torch.cat(val_targets)
            
            preds = all_val_output.argmax(dim=1)
            truth = all_val_targets.argmax(dim=1)
            ...
            _, cra_perc, _ = compute_certificates_CRA(
                all_val_images, model, args.epsilon, correct_indices, norm=args.cra_norm, L=1.0
            )
            global_cra = (len(correct_indices) / len(all_val_targets)) * (cra_perc / 100.0)

            writer.add_scalar("Val/Accuracy", val_acc, epoch)
            writer.add_scalar("Val/CRA", global_cra * 100, epoch)
            val_metrics_str = f"| Val Acc {val_acc:.3f} | Val CRA {global_cra*100:.2f}%"
    ...
    # ==========================================
    print("\n--- Training Complete. Running final evaluation on unseen Test Set ---")
    model.eval()
    optimizer.eval()
    ...
    test_preds = all_test_output.argmax(dim=1)
    test_truth = all_test_targets.argmax(dim=1)
    final_test_acc = (test_preds == test_truth).float().mean().item()
    
    test_correct_indices = (test_preds == test_truth).nonzero(as_tuple=False).squeeze()
    _, final_cra_perc, _ = compute_certificates_CRA(
        all_test_images, model, args.epsilon, test_correct_indices, norm=args.cra_norm, L=1.0
    )
    final_test_cra = (len(test_correct_indices) / len(all_test_targets)) * (final_cra_perc / 100.0)

    print(f"FINAL TEST RESULTS: Accuracy {final_test_acc:.3f} | CRA {final_test_cra*100:.2f}%\n")

    # --- Final Export ---
    if not args.no_save:
        # REPLACE THE HARDCODED PATH WITH THIS:
        LOCAL_MODEL_DIR = args.save_dir 
        os.makedirs(LOCAL_MODEL_DIR, exist_ok=True)
        
        # Using the unbiased test metrics for the filename
        base_filename = f"{args.model_name}_{run_id}_TestAcc{final_test_acc:.2f}_TestCRA{final_test_cra*100:.2f}"
